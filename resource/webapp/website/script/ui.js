function openModal(title,bodyHtml,onSave,saveLabel='Save'){
  ...
  document.getElementById('modal-footer').innerHTML=btn('Cancel','btn btn-s','onclick="closeModal()"')+btn(saveLabel,'btn btn-p','onclick="modalSave()"');
  modalCb=onSave;
  document.getElementById('modal').classList.add('open');
}
function closeModal(){document.getElementById('modal').classList.remove('open');modalCb=null;}
function modalSave(){if(modalCb)modalCb();}

/* ══════════════════════════════════════════
   TOAST
══════════════════════════════════════════ */
function showToast(msg,color='var(--acc)'){
  const t=document.createElement('div');
  t.textContent=msg;
  t.style.cssText=`position:fixed;bottom:88px;right:24px;background:${color};color:#fff;padding:10px 18px;border-radius:8px;font-size:13px;z-index:3000;box-shadow:0 4px 20px rgba(0,0,0,.3);font-family:'DM Sans',sans-serif;font-weight:500;transition:opacity .3s;pointer-events:none`;
...
function buildSidebar(){
  ...
  NAV_ITEMS.forEach(item=>{
    ...
    b.innerHTML=svgIcon(item.icon,16)+`<div class="sb-tip">${item.label}</div>`;
    b.addEventListener('click',()=>navigate(item.id));
    sb.insertBefore(b,bot);
  });
}
...
(function setupBlockLibrary(){
  const origBuild=buildBlockLibraryUI;
  buildBlockLibraryUI=function(){
    const listEl=document.getElementById('bl-lib-list');
    if(!listEl) return;
    listEl.innerHTML=BLOCK_CATALOG.map(group=>`
      <div class="bl-block-group">
        <div class="bl-block-group-label">${group.group}</div>
        ${group.blocks.map(block=>`
          <div class="bl-block-item"
            onclick="openTmplPicker('${block.type}')">
            <div class="bl-block-ic">${svgIcon(block.icon,13)}</div>
            <div class="bl-block-label">${block.label}</div>
            <div class="bl-block-arrow">›</div>
          </div>`).join('')}
      </div>`).join('');
    renderBlockList();
  };
